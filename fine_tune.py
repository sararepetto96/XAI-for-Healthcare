...
from models_explainability import ViT
...
def download_MedVit():
    ...
    if not os.path.exists('MedVit/pretrained/medvit_large.pth'):
        ...
        makedirs("MedVit/pretrained", exist_ok=True)

        import urllib.request

        urllib.request.urlretrieve(model_urls["medvit_large"], f"MedVit/pretrained/medvit_large.pth")
    else:
...
if __name__ == "__main__":
    ...
    datasets = ['dermamnist', 'octmnist', 'pneumoniamnist']
    
    models = ['MedVit', 'vgg16', 'densenet121','convnext_base', 'resnet50',
            'custom_vit_base_patch16_224',
            'custom_deit_base_patch16_224',
            'custom_pit_b_224']
    ...
    parser.add_argument("--corruption_type", type=str, help="Corruption type to use for the dataset preparation.", choices=["jpeg_compression", "speckle_noise", "contrast_down"])
    parser.add_argument("--prepare_data", action=argparse.BooleanOptionalAction, default=False, help="prepare the dataset. Default: no")
    
    #Fine tuning arguments
    parser.add_argument("--model", type=str, help="Model to be fine tuned.", choices=models)
    ...
    if not args.prepare_data and not args.fine_tune:
        print("You need to specify at least one of the following options: --prepare_data or --fine_tune")
        exit(1) 
    ...
    if not args.silent:
        ...
        - Dataset options:
            ...
            - Corruption type: {args.corruption_type} -> The corruption type to use for the dataset preparation.
    ...
    if args.corrupt and args.corruption_type is None:
        print("You need to specify a corruption type when using --corrupt option.")
        exit(1)
