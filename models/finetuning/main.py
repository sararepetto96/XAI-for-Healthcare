...
def get_args_parser():
    ...
    parser.add_argument('--nb_classes', type=int, help='classes in custom dataset')
    parser.add_argument('--eval_data_path', type=str, help='Evaluation dataset path')
...
def main(args):
    ...
    if args.model.startswith('MedViT'):
    ...
    if args.resume:
        if args.model.startswith('MedViT'):
            ...
            raise NotImplementedError("Model--resume is not supported for now.")
    ...
    if args.eval: 
        ...
        """
        loader = torch.utils.data.DataLoader(, batch_size=batch_size, pin_memory=True, shuffle=False)
        
        model_without_ddp.eval().cuda()

        with torch.cuda.amp.autocast():
            with torch.inference_mode():
                correct = 0
                total = 0
                for inputs, labels in loader:  # Use the same validation set
                    inputs, labels = inputs.cuda(), labels.cuda()
                    outputs = model_without_ddp(inputs)
